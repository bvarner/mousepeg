...
\item[\tx{-D}]Display the grammar. Optional.\newline
    ...
    \tx{f}= may fail, \tx{e}= ends processing, \tx{v}=void.)
...
\item[\tx{-L}] 
    Display data about left recursion. Optional.

\item[\tx{-d}]Display the dual grammar created for left-recursion processing. Optional.

\item[\tx{-r}]Display only the rules of dual grammar. Optional.
\eul
...
as explained in Sections \ref{back} and 
\ref{packrat}.
The \textit{arguments} are specified as options according to POSIX syntax:
%
\ul
...
\medskip
Invokes \textsl{PEG Explorer} to analyze the grammar contained in file \textit{filename}.
The \textit{filename} need not be a complete path, 
just enough to identify the file 
in current environment.
It should include file extension, if any. 
