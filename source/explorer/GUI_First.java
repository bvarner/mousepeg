...
//      Use 'Dual.index' instead of 'PEG.index'.
//      Use 'asString' from PEG.Expr instead of 'named'.
//    Version 2.3
...
public class GUI_First extends GUI_List
{
  ...
  {
    startExpr = expr;
    setTitle("First  (" + startExpr.asString + ")");
  ...
  void keyPressed(String command)
    {
      switch(command)
      {
        ...
        case "More":
        {
          ...
          {
            lineNr++;
            Line_Expr newLine = new Line_Expr(Dual.index[iter.next()],theLine.indent+2);
